...
### Jacqueline Grohs, M. Eng. ![Jacqueline Grohs, M. Eng.](./bilder/Jacqueline Grohs_8723.jpg)
...
Die Funktion $y=f(x)$ besitzt an der Stelle $x_{0}$ eine **Nullstelle**, wenn
$$
f(x_{0}) = 0
$$
ist.
...
Für $f(x)=x$ wächst $f(x)$ für $x \to \infty$ ebenfalls gegen Unendlich und für $x \to -\infty$ gegen "minus Unendlich"
...
Für $f(x)=\frac{1}{x}$ wird $f(x)$ gegen 0 gehen und für $x \to 0$ gegen "plus Unendlich"
...
Für ganz-rationale Funktionen vom Grad $\geq 3$ gibt es keine oder nur sehr komplizierte Formeln, aber es hilft der:
...
$$
mit reellen *Koeffizienten* $a_{i}, b_i \in \mathbb{R}$ und $m\geq 1$ heißt **gebrochen-rationale Funktion**.
Die Funktionen $Z(x)$ ist ein Polynom vom Grad $n$ und heißt **Zählerpolynom**, die Funktion $N(x)$ ist ein Polynom vom Grad $m$ und heißt **Nennerpolynom**.
...
Jede unecht gebrochen-rationale Funktion kann durch Polynomdivision in die Summe eine ganz-rationalen Funktion und einer echt gebrochen-rationale Funktion zerlegt werden.

Beispiel: Die unecht gebrochen-rationale Funktion
$$
f(x) = \frac{x^3 - 3x +5}{x-2} = \left(x^2 +2x + 1 \right) + \frac{7}{x-2}
$$
lässt sich durch Polynomdivision in eine ganz-rationalen Teil $x^2+2x+1$ und eine echt gebrochen-rationalen Teil $\frac{7}{x-2}$ zerlegen.

## Nullstellen gebrochen-rationaler Funktionen

Die *Nullstellen* gebrochen-rationaler Funktionen ergeben sich als diejenigen Nullstellen $x$ des Zählerpolynoms $Z(x)$, für die die Funktion $f$ überhaupt definiert ist, also für die $x \in D(f)$ ist. 
...
so heißt dieser Grenzwert die **Ableitung von $f$** oder der **Differenzialquotient von $f$ an der Stelle $x_{0}$**.
...
Die erste Ableitung $f'(x_{0})$ gibt die Steigung der Tangente an den Graphen der Funktion $f$ an der Stelle $x_{0}$ an. 

Damit können also Aussagen gemacht werden über

- das Monotonieverhalten der Funktion $f$ und damit auch über
- evtl. Maxima und Minima der Funktion $f$.
...
f'(x) = g'(h(x)) \cdot h'(x)
$$
Man nennt dabei $g'(h(x))$ die **äußere Ableitung** und $h'(x)$ die **innere Ableitung**. *Beachten Sie, dass in die Ableitung von $g$ der Funktionswert $h(x)$ eingesetzt wird!*
...
Beispiel:
Die Unternehmerin I. bietet exklusiv ein neues Produkt P an. (Sie ist also Monopolistin.)
...
Die **Elastizität** $\varepsilon_{f}(x_0)$ der Funktion $f$ an der Stelle $x_0$ ist definiert als
$$
...
Wir versuchen nun zuerst, das $x_1$ in der zweiten und dritten Gleichung *verschwinden* zu lassen.
Dazu
- addiert man das 5-fache der ersten Gleichung zur zweiten (und erhält so eine neue zweite Gleichung)
- subtrahiert man das 6-fache der ersten Gleichung von der dritten (und erhält so eine neue dritte Gleichung)
...
Die letzte Zeile bedeutet dabei $0 x_1 + 0x_2 + 0x_3 = 0$ und ist *immer* korrekt und liefert damit keine weitere Bedingung. Daher kann man hier zum Beispiel die dritte Variable $x_3$ beliebig wählen.

\tiny Gauß-Algorithmus: Tausche erste und dritte Zeile, addiere dann die erste zur zweiten, subtrahiere dann die zweite von der dritten.
\normalsize
...
\tiny Gauß-Algorithmus: Zweite minus 2-mal erste und dritte minus erste, danach dritte minus zweite.
\normalsize
...
\tiny Gauß-Algorithmus: Zweite plus viermal dritte Zeile und die zweite durch 3 teilen, erste minus zweimal die dritte Zeile, und danach noch die zweite zur ersten addieren.
\normalsize
...
\tiny Gauß-Algorithmus: Da unten bereits eine Null steht: Erste Zeile zur zweiten addieren, danach zweite von der dritten subtrahieren.
\normalsize
